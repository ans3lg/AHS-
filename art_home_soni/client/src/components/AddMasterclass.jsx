...
const AddMasterclass = ({ setNotification }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const confirmAdd = window.confirm("Добавить мастер-класс?");
    if (!confirmAdd) return;

    try {
      const response = await fetch("http://localhost:5000/api/masterclasses", {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        setNotification({ message: "Мастер-класс добавлен!", type: "success" });
      } else {
        setNotification({ message: result.message || "Ошибка", type: "error" });
      }

      setTimeout(() => setNotification({ message: "", type: "" }), 3000);
    } catch (error) {
      console.error("Ошибка при добавлении:", error);
      setNotification({ message: "Ошибка сервера", type: "error" });
      setTimeout(() => setNotification({ message: "", type: "" }), 3000);
    }
  };
